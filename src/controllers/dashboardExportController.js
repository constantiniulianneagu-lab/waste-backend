...
// =============================================================================
// Area chart (line + soft fill) with intermediate Y grid labels
// =============================================================================
function drawAreaChart(doc, x, y, w, h, monthlyEvolution, FONT_REG, FONT_BOLD) {
  ...
  // choose tick step and ticks count
  const step = niceStep(max, 4); // target ~4 intervals
  const top = Math.ceil(max / step) * step;
  const ticks = Math.max(4, Math.round(top / step)); // number of intervals
  const lines = ticks; // grid lines count
  ...
  // points (scaled to top, not max, so grid matches)
...
function drawWasteCodesTable(doc, x, y, w, h, rows, FONT_REG, FONT_BOLD) {
  ...
  let cy = startY + 22;
  const rowH = 26; // taller to fit description
...
// Header: Sector | Tichete | Cantitate (t)
// =============================================================================
function drawSectorsTable(doc, x, y, w, h, sectors, FONT_REG, FONT_BOLD) {
  ...
  // sectors already normalized Sectorul 1..6
...
// =============================================================================
// Operators table (no external title)
// Header: Operator salubrizare | Sector | Cantitate (t)
// =============================================================================
